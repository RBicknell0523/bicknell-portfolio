...
const PreviewGeneratedText = ({ generatedContent, height }: Props) => {
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-8 pb-8 pt-5 lg:col-span-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="pb-2 text-2xl font-bold text-foreground">Output Result</h2>
          <p>Enjoy your outstanding content based on your topic</p>
        </div>
        <button
          onClick={copyToClipboard}
          className="relative mt-9 inline-flex items-center gap-1.5 rounded-lg border border-border bg-muted px-6 py-3 text-sm text-foreground transition-colors hover:bg-accent"
        >
          <svg
            width="16"
            height="16"
      ...
      <textarea
        className={`mt-6 w-full rounded-lg border border-border bg-input p-5 text-foreground outline-hidden focus:border-ring ${
          height === 442 ? "min-h-[442px]" : "min-h-[262px]"
        } ${!generatedContent && "cursor-no-drop"}`}
        value={
          generatedContent
            ? generatedContent
